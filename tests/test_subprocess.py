...
from c4f.utils import SubprocessHandler, SecureSubprocess, ProcessResourceMonitor, SecureSubprocessTermination, \
    FileChange
...
class TestSubprocessHandler:
    ...
    def test_process_output_binary_mode(self, subprocess_handler, mock_process):
        ...
        result_stdout, result_stderr, result_code = subprocess_handler._process_output(
            stdout, stderr, is_text_mode=False, encoding='utf-8', errors='replace', process=mock_process
        )
    ...
    def test_process_output_text_mode(self, subprocess_handler, mock_process):
        ...
        result_stdout, result_stderr, result_code = subprocess_handler._process_output(
            stdout, stderr, is_text_mode=True, encoding='utf-8', errors='replace', process=mock_process
        )
    ...
    def test_cleanup_process_with_closed_fds(self, subprocess_handler, mock_process):
        ...
        mock_process.stdout.close.side_effect = IOError("Already closed")
        mock_process.stderr.close.side_effect = IOError("Already closed")
    ...
    def test_handle_execution_error_with_termination_error(self, subprocess_handler, mock_process):
        ...
        with pytest.raises(OSError, match="Test error"):
            subprocess_handler._handle_execution_error(mock_process, OSError("Test error"))
        
        # Verify termination was attempted
        mock_process.terminate.assert_called_once()

    def test_cleanup_process_with_termination_error(self, subprocess_handler, mock_process):
        """Test _cleanup_process when process termination fails."""
        # Configure mock to raise OSError during termination
        mock_process.terminate.side_effect = OSError("Termination failed")
    ...
    def test_terminate_process_with_psutil_error(self, subprocess_handler, mock_process):
    ...
    def test_monitor_process_tree_with_psutil_error(self, subprocess_handler, mock_process):
    ...
    def test_process_output_binary_mode_with_non_bytes(self, secure_subprocess, mock_process):
...
class TestSecureSubprocess:
    ...
    def test_secure_subprocess_execute_with_exception(self, secure_subprocess):
        ...
        with patch.object(secure_subprocess, '_execute_subprocess', return_value=('', '', -1)):
            # Need to patch _prepare_command to avoid validation error
            with patch.object(secure_subprocess, '_prepare_command', return_value=ECHO_CMD):
                # Set up mock for _prepare_text_mode_kwargs
                with patch.object(secure_subprocess, '_prepare_text_mode_kwargs', return_value={}):
                    # Call _run_secure_subprocess 
                    result = secure_subprocess._run_secure_subprocess(ECHO_CMD, is_text_mode=True)
    ...
    def test_secure_subprocess_termination_with_error(self, mock_process):
        ...
        terminator = SecureSubprocessTermination(termination_wait=0.01)  # Use shorter wait time for testing
        
        # Configure mock process poll to return None on first call, 0 on second call
        poll_return_values = [None, 0]  # First call: still running, second call: terminated
        mock_process.poll.side_effect = lambda: poll_return_values.pop(0) if poll_return_values else 0
    ...
    def test_secure_subprocess_init_with_psutil_warning(self):
        ...
        with patch("c4f.utils.PSUTIL_AVAILABLE", False):
            ...
            with patch("c4f.utils.logger") as mock_logger:
                ...
                SecureSubprocess(cpu_limit=10.0, memory_limit=1024 * 1024)
...
class TestProcessResourceMonitor:
    ...
    def test_check_resource_limits_no_exceed(self, resource_monitor):
        ...
        mock_psutil_process.memory_info.return_value = MagicMock(rss=512 * 1024)  # 512KB
        
        # Should return False
        assert resource_monitor._check_resource_limits([mock_psutil_process], mock_psutil_process) is False
...
class TestSecureSubprocessTermination:
    ...
    def test_terminate_process_already_terminated(self, termination_handler, mock_process):
    ...
    def test_perform_standard_termination_with_error(self, termination_handler, mock_process):
...
class TestEdgeCases:
    ...
    def test_unicode_decode_error_fallback(self, tmp_path, subprocess_handler):
        ...
        with patch.object(subprocess_handler, "run_text_mode") as mock_text_mode:
            ...
            mock_text_mode.side_effect = UnicodeDecodeError('utf-8', b'\x80\x81', 0, 1, 'invalid start byte')
            
            # Patch run_binary_mode to return a known value
            with patch.object(subprocess_handler, "run_binary_mode") as mock_binary_mode:
    ...
    def test_execute_subprocess_fallback_return(self, subprocess_handler):
        ...
        with patch.object(subprocess_handler, "_start_process") as mock_start:
            ...
            with patch.object(subprocess_handler, "_handle_execution_error") as mock_handle:
                mock_handle.return_value = None  # Don't raise an exception
                
                # Call _execute_subprocess
                stdout, stderr, returncode = subprocess_handler._execute_subprocess(
                    ["test"], {}, None, True, 'utf-8', 'replace'
                )
    ...
    def test_cpu_limit_exceeded(self, mock_process):
        ...
        # Create a mock psutil.Process
        mock_psutil_process = MagicMock()
        mock_psutil_process.cpu_percent.return_value = 90.0  # 90% CPU usage (exceeds limit)
        
        # Check CPU limit
        assert monitor._check_cpu_limit(mock_psutil_process, mock_psutil_process) is True
    ...
    def test_memory_limit_exceeded(self, mock_process):
        ...
        mock_psutil_process = MagicMock()
        mock_psutil_process.memory_info.return_value = MagicMock(rss=2 * 1024 * 1024)  # 2MB (exceeds limit)
        
        # Check memory limit
        assert monitor._check_memory_limit(mock_psutil_process, mock_psutil_process) is True
    ...
    def test_run_binary_mode_with_override(self, secure_subprocess):
        ...
        with patch.object(secure_subprocess, "_run_secure_subprocess") as mock_run:
            ...
            mock_run.assert_called_once_with(
                ECHO_CMD, is_text_mode=False, encoding="latin1", errors="ignore", timeout=99
            )
    ...
    def test_secure_subprocess_termination_handler(self, secure_subprocess, mock_process):
        """Test the termination handler in SecureSubprocess."""
        # Test direct call to _terminate_process_and_children
        if PSUTIL_AVAILABLE:
            with patch.object(secure_subprocess.termination_handler, "terminate_process_and_children") as mock_terminate:
                mock_psutil_process = MagicMock()
        ...
        with patch.object(secure_subprocess.termination_handler, "terminate_process") as mock_terminate:
    ...
    def test_log_messages(self, secure_subprocess, caplog):
        ...
        with patch.object(secure_subprocess, "validate_command") as mock_validate:
            ...
            assert any(record.levelname == "INFO" and "Executing command" in record.message for record in caplog.records)
...
class FileChangeHandler:
    ...
    def handle_file_change(self, file_path: str) -> Optional[FileChange]:
        ...
        try:
            ...
            import warnings
            warnings.warn(f"Error handling file change: {str(e)}")
            return None
        ...
        try:
            while self._monitoring and self.process.poll() is None:
                try:
                    ...
                    import warnings
                    warnings.warn(f"Error during process monitoring: {str(e)}")
                    break
        except OSError:
